...
enum Repr {
    ...
    // Panic is captured with the `dyn Any + Send` type, which is !Sync. The
    // SyncWrapper makes the type Sync by disallowing immutable references.
