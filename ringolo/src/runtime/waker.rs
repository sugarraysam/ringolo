...
/// Waking interface for the root future, i.e.: the one passed to `block_on`.
/// We implement this interface on scheduler Handles, which sit behind an Arc.
/// It will only be used to indicate if the root future has been woken up. Other
/// spawned tasks have a separate waker implementation defined in the `task/`
/// module.
pub(super) trait Wake: Send + Sync + Sized + 'static {
    /// Wake by value.
...
pub(super) fn waker_ref<W: Wake>(wake: &Arc<W>) -> WakerRef<'_> {
...
pub(super) fn waker<W: Wake>(wake: Arc<W>) -> Waker {
    unsafe {
